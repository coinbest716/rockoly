...
export const ChefList = props => {
  ...
  const [updatefollowlist, { data }] = useMutation(UPDATE_FOLLOW_UNFOLLOW_LIST, {
    onCompleted: data => {
      // setLoading(false);
      toastMessage('success', 'Added to Favourite List');
    },
    onError: err => {
      ...
      {chefdetails &&
        chefdetails.map(chef => {
          if (isObjectEmpty(chef) && isStringEmpty(chef.fullName)) {
            return (
              <div className="col-lg-3 col-sm-6 col-md-4 col-6 products-col-item" key={chef.chefId}>
                  ...
                  <div className="product-content" onClick={() => onSelectDetail(chef)}>
                    ...
                    {chef &&
                      ...
                      chef.chefProfileExtendedsByChefId.nodes.map(node => {
                        return (
                          <div>
                            {isObjectEmpty(node) && isStringEmpty(node.chefAddrLine2) ? (
                              <p className="address">{node.chefAddrLine2}</p>
                            ) : (
                              <p>-----</p>
                            )}
                            {isObjectEmpty(node) &&
                    ...
                    {chef.totalReviewCount > 0 ? (
                      <div>{chef.totalReviewCount} Reviews</div>
                    ) : (
                      <div>No Reviews</div>
                    )}
                    <br />
